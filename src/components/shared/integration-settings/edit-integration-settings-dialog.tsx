...
import { updatePlatformIntegrationSettingsFn } from '@/server/settings/platform-settings.fns'
...
export function EditIntegrationSettingsDialog({
  ...
  // Both hooks run unconditionally (they are hooks); which endpoint is used is
  // the selection below. The two are guarded differently server-side —
  // requirePlatformAdmin vs requireAdmin(INTEGRATIONS_MANAGE) — so picking the
  // wrong one fails closed rather than writing the wrong scope.
  const updateAgency = useServerFn(updateIntegrationSettingsFn)
  const updatePlatform = useServerFn(updatePlatformIntegrationSettingsFn)
  const updateSettings = scope === 'platform' ? updatePlatform : updateAgency
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: [SETTINGS_QUERY_KEY[scope]] })
      onOpenChange(false)
    },
    onError: (err) =>
